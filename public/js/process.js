...
function initSocket(visualiseProcess) {
	socket = io();
	socket.emit('scanProcess');
	//socket.emit('scores');

	socket.on('scanProcess', function(data) {
		if (scanId != data){
			 scanId = data;
			 console.log(data);
			 socket.emit('streamProcess', scanId);
		}
	});

	socket.on('streamProcess', function(data) {
		streamAction(data);
	});

	function streamAction(data){
		var processArray = JSON.parse(data).processes;
		console.log(data);

		try {
			var time = JSON.parse(data)._id.split('-')[0];
		...
		catch(err) {} // Catch undefined length
	}
